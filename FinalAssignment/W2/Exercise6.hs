...
exercise6 = do
  ...
  putStrLn "\bLocal test string for personal testing.\n"
  print(testingString "Testing this string")
  
  putStrLn "\bQuickcheck to check if the inversion of the inversed property equals to the actual string.\n"
  quickCheck prop_is_inversed

  putStrLn "\bQuickcheck to check if the inversion stays the same when there are only non alphabetic letters.\n"
