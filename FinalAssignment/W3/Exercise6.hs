...
-- We can assume that the input form is in conjunctive normal form, 
...
clsPropNames c = nub (map abs (concat c))

-- Function to compare the cls properties to the properties of the cnf of the same form
prop_sameProperties :: Form -> Clauses -> Bool
prop_sameProperties f c = clsPropNames c == propNames (cnf f)
...
testAmountOfClauses :: Form -> Clauses -> Bool
testAmountOfClauses f c = countClauses (cnf f) == length c
...
exercise6 = do
  ...
  print $ prop_sameProperties form1 (cls form1)
